...
class GitHubSourceAdapter {
  ...
  static async connect(site,cred){
    ...
    const api=async(path,{method='GET',body,allow404=false}={})=>{
      const res=await fetch('https://api.github.com/repos/'+repo+path,{method,headers:{accept:'application/vnd.github+json',authorization:'Bearer '+cred.token,'x-github-api-version':'2022-11-28','user-agent':'Lorzen-SiteOps/0.6.1'},body:body===undefined?undefined:JSON.stringify(body),signal:AbortSignal.timeout(30000)});
...
async function gh(path,{method='GET',body,allow404=false}={}){
  const res=await fetch('https://api.github.com'+backupRepoPath(path),{
    ...
    headers:{
      ...
      'user-agent':'Lorzen-SiteOps/0.6.1'
    },
...
function setupPage(){
  return page('Website hinzufügen',`
    ...
    function syncDeployment(){
      const git=deployment.value==='hostinger_git';webspaceSection.hidden=git;gitSection.hidden=!git;webspaceSection.dataset.visible=git?'false':'true';gitSection.dataset.visible=git?'true':'false';
