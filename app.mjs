...
class GitHubSourceAdapter {
  ...
  static async connect(site,cred){
    ...
    const api=async(path,{method='GET',body,allow404=false}={})=>{
      const res=await fetch('https://api.github.com/repos/'+repo+path,{method,headers:{accept:'application/vnd.github+json',authorization:'Bearer '+cred.token,'x-github-api-version':'2022-11-28','user-agent':'Lorzen-SiteOps/0.7.1'},body:body===undefined?undefined:JSON.stringify(body),signal:AbortSignal.timeout(30000)});
...
async function fetchWithRedirectTrace(url,timeoutMs){
  ...
  for(let i=0;i<8;i++){
    response=await fetch(current,{redirect:'manual',signal:AbortSignal.timeout(timeoutMs),headers:{'User-Agent':'Lorzen-SiteOps/0.7.1'}});
...
async function checkSiteNow(site){
  ...
  try{
    ...
    if(!error&&site.monitor_check_wordpress){
      try{
        const origin=new URL(finalUrl).origin,wpRes=await fetch(origin+'/wp-json/',{redirect:'follow',signal:AbortSignal.timeout(site.monitor_timeout_ms),headers:{'User-Agent':'Lorzen-SiteOps/0.7.1'}});
...
function mcpInfoPage(){
  ...
  return page('MCP',`<header><div><span class="eyebrow">AI / AUTOMATION</span><h1>MCP</h1><p>SiteOps stellt ChatGPT, Claude und anderen MCP-Clients die Website-Verwaltung kontrolliert als Tools bereit.</p></div><span class="pill ${cfg.mcpToken?'ok':'bad'}">${cfg.mcpToken?'AUTH KONFIGURIERT':'TOKEN FEHLT'}</span></header>
  <div class="twocol ops-grid"><section><h2>Verbindung</h2><dl class="facts"><div><dt>Endpoint</dt><dd><code>${esc(endpoint)}</code></dd></div><div><dt>Transport</dt><dd>Streamable HTTP</dd></div><div><dt>Authentifizierung</dt><dd>Bearer Token</dd></div><div><dt>Version</dt><dd>0.7.1</dd></div></dl><p class="lead">Der MCP_API_TOKEN wird hier absichtlich nicht angezeigt. Er wird ausschließlich für die Authentifizierung des MCP-Endpunkts verwendet.</p></section>
  <section><h2>Empfohlener Ablauf</h2><ol class="workflow"><li><code>site_overview</code> für Kontext</li><li><code>files_find</code> / <code>text_search</code> zur Orientierung</li><li><code>file_read</code> für relevante Dateien</li><li><code>change_preview</code> für den Vorschlag</li><li>Freigabe durch den Nutzer</li><li><code>change_apply</code></li><li><code>site_status</code> zur Kontrolle</li></ol></section></div>
  <section><div class="sectionhead"><div><span class="eyebrow">Tools</span><h2>Verfügbare Bereiche</h2></div></div><div class="tool-grid">${groups.map(([name,tools])=>`<div class="tool-card"><strong>${name}</strong><p>${tools.split(', ').map(t=>'<code>'+t+'</code>').join(' ')}</p></div>`).join('')}</div></section>
  <div class="notice"><strong>Sicherheitsmodell</strong><span>Secrets werden nie über MCP zurückgegeben. Dateiänderungen bleiben zweistufig: Preview zuerst, Apply erst nach Freigabe. Verbindungsänderungen werden vor dem Speichern getestet.</span></div>`);
}
...
});const site=await updateSite(req.params.site,schema.parse(req.body));return{ok:true,site:publicSite(site)};});app.post('/api/backups/:id/restore-preview',async req=>backupRestorePreview(req.params.id,'dashboard'));app.post('/api/changes/:id/rollback-preview',async req=>rollbackPreview(req.params.id,'dashboard'));app.post('/api/previews/:id/apply',async req=>applyPreview(req.params.id));app.get('/mcp-info',async(_r,reply)=>reply.type('text/html').send(mcpInfoPage()));app.get('/settings',async(_r,reply)=>reply.type('text/html').send(settingsPage()));app.get('/setup',async(_r,reply)=>reply.type('text/html').send(setupPage()));app.get('/sites/:slug',async(req,reply)=>reply.type('text/html').send(await siteOperationsPage(req.params.slug)));app.get('/incidents/:id',async(req,reply)=>reply.type('text/html').send(await incidentPage(req.params.id)));if(databaseReady){startMonitor();startBackupWorker();}await app.listen({host:cfg.host,port:cfg.port});app.log.info({port:cfg.port,host:cfg.host,databaseReady},'SiteOps listening');}
