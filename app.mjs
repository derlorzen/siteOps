...
function settingsPage(){
  ...
  return page('Einstellungen',`
    ...
    testPageSpeed.onclick=async()=>{pageSpeedStatus.textContent='Prüfe…';const p=payload();const r=await fetch('/api/settings/pagespeed-test',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({pageSpeedApiKey:p.pageSpeedApiKey,url:p.publicBaseUrl})}),x=await r.json();pageSpeedStatus.textContent=r.ok?'✓ Performance '+(x.result?.scores?.performance??'–')+' · SEO '+(x.result?.scores?.seo??'–'):'✗ '+(x.message||x.error||JSON.stringify(x));};
    </script>`);
}
function setupPage(){
