...
async function siteOperationsPage(slug){
  ...
  return page(site.name,`
    ...
    ${latest&&!latest.ok?`<div class="notice bad"><strong>Monitoringfehler</strong><span>${esc(latest.error||latestDetails.error||('HTTP '+(latest.http_status??'unbekannt')))}</span><button class="ghost" onclick="siteOpsCopyPrompt({kind:'monitor',site:${JSON.stringify(site.slug)}},this)">Fix-Prompt kopieren</button></div>`:''}
    ...
    ${backupState?.last_error?`<div class="notice bad"><strong>Backupfehler</strong><span>${esc(backupState.last_error)}</span><button class="ghost" onclick="siteOpsCopyPrompt({kind:'backup',site:${JSON.stringify(site.slug)}},this)">Fix-Prompt kopieren</button></div>`:''}
