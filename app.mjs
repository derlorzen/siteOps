...
function setupPage(){
  return page('Website hinzufügen',`
    ...
    <script>
    const form=document.getElementById('siteForm'),protocol=form.protocol,port=form.port,domain=form.domain,slug=form.slug,deployment=form.deploymentMode;
    const webspaceSection=document.getElementById('webspaceSection'),gitSection=document.getElementById('gitSection'),connectionHelp=document.getElementById('connectionHelp');
    ...
    function syncDeployment(){
      const git=deployment.value==='hostinger_git';webspaceSection.hidden=git;gitSection.hidden=!git;
      testConnection.textContent=git?'GitHub-Quelle testen':'Webspace-Verbindung testen';
      connectionHelp.textContent=git?'Prüft Repository, Branch und Schreibzugriff auf das GitHub-Quell-Repository.':'Teste den Webspace, bevor du die Website speicherst.';
      connectionStatus.textContent='';connectionFiles.innerHTML='';
    }
    deployment.addEventListener('change',syncDeployment);syncDeployment();
    function data(){
      const fd=new FormData(form),git=fd.get('deploymentMode')==='hostinger_git';
      return {name:fd.get('name').trim(),domain:fd.get('domain').trim().replace(/^https?:\/\//,'').replace(/\/$/,''),slug:fd.get('slug').trim(),
        siteType:fd.get('siteType'),deploymentMode:fd.get('deploymentMode'),
        protocol:fd.get('protocol'),host:fd.get('host').trim(),port:Number(fd.get('port')),username:fd.get('username').trim(),password:fd.get('password'),remoteRoot:fd.get('remoteRoot').trim()||'/',
        sourceRepository:fd.get('sourceRepository').trim().replace(/^\/+|\/+$/g,''),sourceBranch:fd.get('sourceBranch').trim()||'main',sourceRoot:fd.get('sourceRoot').trim().replace(/^\/+|\/+$/g,''),gitToken:fd.get('gitToken'),hostingerTargetDirectory:fd.get('hostingerTargetDirectory').trim()||'public_html',
        backupEnabled:form.backupEnabled.checked,backupIntervalSeconds:Math.round(Number(fd.get('backupHours'))*3600),backupMaxFiles:Number(fd.get('backupMaxFiles')),
        monitorEnabled:form.monitorEnabled.checked,monitorIntervalSeconds:Number(fd.get('monitorIntervalSeconds')),monitorFailureThreshold:Number(fd.get('monitorFailureThreshold')),sslWarnDays:Number(fd.get('sslWarnDays'))};
    }
    testConnection.onclick=async()=>{connectionStatus.textContent='Prüfe…';connectionFiles.innerHTML='';const r=await fetch('/api/site-connection-test',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(data())});const x=await r.json();if(!r.ok){connectionStatus.textContent='✗ '+(x.message||x.error||JSON.stringify(x));return;}connectionStatus.textContent='✓ '+(x.mode==='hostinger_git'?'GitHub-Quelle erreichbar':'Verbindung erfolgreich');const prefix=x.repository?'<strong>'+x.repository+' · '+x.branch+'</strong><br>':'';connectionFiles.innerHTML=prefix+(x.entries.length?'Gefundene Einträge: '+x.entries.map(e=>(e.type==='directory'?'Ordner: ':'Datei: ')+e.name).join(' · '):'Verbindung erfolgreich, Ordner ist leer.');};
