...
async function seoPageDetailPage(id){
  ...
  let termRows='';terms.forEach(t=>termRows+='<tr data-row data-search="'+esc(String(t.term||'').toLowerCase())+'"><td><strong>'+esc(t.term)+'</strong></td><td>'+t.freq+'</td><td>'+t.wdf+'</td><td>'+t.idf+'</td><td>'+t.score+'</td></tr>');if(!termRows)termRows='<tr><td colspan="5">Keine ausreichenden Textdaten.</td></tr>';
  let linkRows='';links.forEach(l=>{const type=l.internal_link?'intern':'extern',rel=l.nofollow?'nofollow':'follow',searchValue=esc(((l.target_url||'')+' '+(l.anchor_text||'')).toLowerCase());linkRows+='<tr data-row data-search="'+searchValue+'" data-type="'+type+'" data-rel="'+rel+'"><td>'+type+'</td><td><a href="'+esc(l.target_url)+'" target="_blank" rel="noopener">'+esc(l.target_url)+'</a></td><td>'+esc(l.anchor_text||'–')+'</td><td>'+rel+'</td></tr>';});if(!linkRows)linkRows='<tr><td colspan="4">Keine Links.</td></tr>';
  ...
  html+='<section><div class="sectionhead"><div><span class="eyebrow">Content</span><h2>Siteinterne WDF×IDF</h2></div><small>Gewichtung relativ zum Korpus der gecrawlten Website – keine Wettbewerberanalyse.</small></div><div class="filterbar"><input id="wdfSearch" type="search" placeholder="Begriff filtern"><select id="wdfPageSize"><option value="10">10 / Seite</option><option value="15" selected>15 / Seite</option><option value="25">25 / Seite</option><option value="50">50 / Seite</option></select></div><div class="tablewrap"><table id="wdfTable"><thead><tr><th>Term</th><th>Häufigkeit</th><th>WDF</th><th>IDF</th><th>WDF×IDF</th></tr></thead><tbody>'+termRows+'</tbody></table></div><div class="pagination-bar" id="wdfPager"></div></section>';
  html+='<section><div class="sectionhead"><div><span class="eyebrow">Verlinkung</span><h2>Ausgehende Links</h2></div><small>'+links.length+' erkannte Links · intern/extern und Follow-Status filterbar.</small></div><div class="filterbar"><input id="linkSearch" type="search" placeholder="Ziel oder Ankertext filtern"><select id="linkType"><option value="all">Intern + extern</option><option value="intern">Nur intern</option><option value="extern">Nur extern</option></select><select id="linkRel"><option value="all">Follow + nofollow</option><option value="follow">Follow</option><option value="nofollow">Nofollow</option></select><select id="linkPageSize"><option value="15">15 / Seite</option><option value="25" selected>25 / Seite</option><option value="50">50 / Seite</option><option value="100">100 / Seite</option></select></div><div class="tablewrap"><table id="linkTable"><thead><tr><th>Typ</th><th>Ziel</th><th>Ankertext</th><th>Rel</th></tr></thead><tbody>'+linkRows+'</tbody></table></div><div class="pagination-bar" id="linkPager"></div></section>';
  html+='<script>window.addEventListener("DOMContentLoaded",()=>{siteOpsTableView({tableId:"wdfTable",searchId:"wdfSearch",pageSizeId:"wdfPageSize",pagerId:"wdfPager",defaultSize:15});siteOpsTableView({tableId:"linkTable",searchId:"linkSearch",pageSizeId:"linkPageSize",pagerId:"linkPager",defaultSize:25,filters:[{id:"linkType",data:"type"},{id:"linkRel",data:"rel"}]});});</script>';
  return page('SEO · '+(p.title||p.path),html);
}

async function siteOperationsPage(slug){
  const site=await getSite(slug),hist=await listHistory(site.id,30),backups=await listBackups(site,30),checks=await listMonitorChecks(site.id,200),incidents=await listIncidents(site.id,30),backupState=(await q('select * from backup_state where site_id=?',[site.id])).rows[0]||null,monitorState=(await q('select * from monitor_state where site_id=?',[site.id])).rows[0]||null;
...
async function dashboard(){
  const sites=await listSites(),
    checks=(await q('select mc.site_id,mc.ok,mc.http_status,mc.response_ms,mc.ssl_days,mc.created_at from monitor_checks mc join (select site_id,max(created_at) created_at from monitor_checks group by site_id) latest on latest.site_id=mc.site_id and latest.created_at=mc.created_at')).rows,
    checkMap=new Map(checks.map(x=>[x.site_id,x])),
    backups=(await q('select b.site_id,b.git_commit,b.file_count,b.created_at from backups b join (select site_id,max(created_at) created_at from backups group by site_id) latest on latest.site_id=b.site_id and latest.created_at=b.created_at')).rows,
    backupMap=new Map(backups.map(x=>[x.site_id,x])),
    incidents=(await q("select i.*,s.domain,s.slug from incidents i join sites s on s.id=i.site_id where i.status='open' order by i.created_at desc")).rows,
    changes=(await q('select c.*,s.domain from changes c join sites s on s.id=c.site_id order by c.created_at desc limit 20')).rows;
  const online=sites.filter(s=>checkMap.get(s.id)?.ok).length,alarms=sites.filter(s=>checkMap.get(s.id)&&!checkMap.get(s.id).ok).length,noData=sites.filter(s=>!checkMap.get(s.id)).length;
  const cards=sites.map(s=>{
    const c=checkMap.get(s.id),b=backupMap.get(s.id),deploy=s.deployment_mode==='hostinger_git'?'HOSTINGER GIT':s.protocol.toUpperCase();
  ...
  const inc=incidents.length?incidents.map(i=>`<li><a href="/incidents/${i.id}"><b>${esc(i.domain)}</b> ${esc(i.title)}</a><small>${new Date(i.created_at).toLocaleString('de-DE')}</small></li>`).join(''):'<li>Keine offenen Incidents.</li>';
  ...
  const focusRows=sites.map(s=>{
    const c=checkMap.get(s.id),b=backupMap.get(s.id),deploy=s.deployment_mode==='hostinger_git'?'Git':'Webspace',state=c?(c.ok?'ONLINE':'ALARM'):'NO DATA';
  ...
  const attention=[
    ...incidents.slice(0,8).map(i=>`<a class="attention-item bad" href="/incidents/${i.id}"><span><strong>Incident · ${esc(i.domain)}</strong><small>${esc(i.title)}</small></span><b>Öffnen →</b></a>`),
    ...sites.filter(s=>checkMap.get(s.id)&&!checkMap.get(s.id).ok).map(s=>`<a class="attention-item bad" href="/sites/${esc(s.slug)}"><span><strong>Monitoring · ${esc(s.name)}</strong><small>Letzter Check meldet Fehler</small></span><b>Prüfen →</b></a>`),
    ...sites.filter(s=>!checkMap.get(s.id)).map(s=>`<a class="attention-item" href="/sites/${esc(s.slug)}"><span><strong>Keine Monitoringdaten · ${esc(s.name)}</strong><small>Für diese Website liegt noch kein Check vor.</small></span><b>Öffnen →</b></a>`)
  ].slice(0,12).join('')||'<div class="empty-state compact-empty"><strong>Kein akuter Handlungsbedarf.</strong><p>Alle vorhandenen Monitoringdaten sind unauffällig.</p></div>';
  const recentFocus=changes.slice(0,6).map(c=>`<li><b>${esc(c.domain)}</b> ${esc(c.description)}<small>${new Date(c.created_at).toLocaleString('de-DE')}</small></li>`).join('')||'<li>Noch keine Änderungen.</li>';
  return page('SiteOps',`
