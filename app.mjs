...
function oauthClientBasic(req){
  ...
  try{const decoded=Buffer.from(h.slice(6),'base64').toString(),i=decoded.indexOf(':');if(i<0)return null;return{clientId:decodeURIComponent(decoded.slice(0,i)),clientSecret:decodeURIComponent(decoded.slice(i+1))};}catch{return null;}
}
...
async function oauthToken(req,reply){
  ...
  try{
    if(grant==='authorization_code'){
      ...
      return reply.send(await oauthIssueTokens({clientId:row.client_id,scope:row.scope,resource:row.resource,subject:row.subject}));
    }
