...
async function loadSavedConfig(){
  ...
  apply('github_backup_repo','githubBackupRepo');
...
async function saveAppSettings(x){
  ...
  for(const [keyName,target] of plain){
    ...
    cfg[target]=x[target];
    await storeSetting(keyName,x[target]);
  }
...
async function testSiteConnection(x){const site={protocol:x.protocol,host:x.host,port:x.port,username:x.username,remote_root:x.remoteRoot,encrypted_credentials:encrypt({password:x.password,privateKey:x.privateKey,passphrase:x.passphrase})},remote=await connectSite(site);try{const entries=await remote.list(joinRemote(site.remote_root,''));return{ok:true,path:site.remote_root,entries:entries.slice(0,12).map(e=>({name:e.name,type:e.type}))};}finally{await remote.close();}}
...
async function gh(path,{method='GET',body,allow404=false}={}){
  const res=await fetch('https://api.github.com'+backupRepoPath(path),{
    ...
    headers:{
      ...
      'user-agent':'Lorzen-SiteOps/0.5.0'
    },
...
function settingsPage(){
  ...
  return page('Einstellungen',`
    ...
    testGitHub.onclick=async()=>{githubStatus.textContent='Prüfe…';const r=await fetch('/api/settings/github-test',{method:'POST'}),x=await r.json();githubStatus.textContent=r.ok?'✓ '+x.repository+' erreichbar und privat':'✗ '+(x.message||x.error||JSON.stringify(x));};
...
function setupPage(){
  return page('Website hinzufügen',`
    <header><div><span class="eyebrow">Neue Website</span><h1>Website hinzufügen</h1><p>SiteOps benötigt den Webspace-Zugang, um Dateien zu lesen, Backups zu erstellen und freigegebene Änderungen sicher einzuspielen.</p></div></header>
    <div class="notice"><strong>SFTP ist die erste Wahl.</strong><span>Nutze FTPS oder unverschlüsseltes FTP nur, wenn dein Hoster kein SFTP anbietet. Zugangsdaten werden mit dem SiteOps-Master-Key verschlüsselt gespeichert.</span></div>
    <form id="siteForm" class="config-form">
      <section class="form-card">
        <div class="sectionhead"><div><span class="eyebrow">1 · Zuordnung</span><h2>Welche Website ist das?</h2></div></div>
        <div class="formgrid">
          ...
          <label>Website-Typ <small>Hilft SiteOps später bei passenden Prüfungen und Ausschlüssen.</small><select name="siteType"><option value="wordpress">WordPress</option><option value="php" selected>PHP / HTML</option><option value="static">Statische Website</option></select></label>
        </div>
      </section>

      <section class="form-card">
        <div class="sectionhead"><div><span class="eyebrow">2 · Webspace</span><h2>Wie erreicht SiteOps die Dateien?</h2></div></div>
        <div class="formgrid">
          ...
          <label>Port <small>SFTP meist 22, FTP/FTPS meist 21.</small><input name="port" type="number" value="22" required></label>
          <label class="span2">Host / Server <small>Der Servername deines Hosters, nicht zwingend die Website-Domain. Beispiel: <code>ssh.example-host.de</code>.</small><input name="host" required placeholder="ssh.example-host.de"></label>
          <label>Benutzername <small>FTP-/SFTP-Benutzer des Webspaces.</small><input name="username" required autocomplete="username"></label>
          <label>Passwort <small>Wird verschlüsselt gespeichert.</small><input name="password" type="password" autocomplete="new-password"></label>
          <label class="span2">Webroot / Remote Root <small>Der Ordner, in dem die Website tatsächlich liegt. Dort sollten z. B. <code>index.php</code>, <code>wp-config.php</code> oder <code>index.html</code> zu finden sein. Wenn der Login direkt dort startet, ist <code>/</code> richtig.</small><input name="remoteRoot" value="/" required placeholder="/"></label>
        </div>
      ...
      <section class="form-card">
        <div class="sectionhead"><div><span class="eyebrow">3 · Betrieb</span><h2>Backup & Monitoring</h2></div></div>
        <p class="lead">Die globalen Standardwerte kommen aus den SiteOps-Einstellungen. Du kannst sie hier bereits für diese Website abweichend setzen.</p>
        <div class="formgrid">
    ...
    <script>
    const form=document.getElementById('siteForm'),protocol=form.protocol,port=form.port,domain=form.domain,slug=form.slug;
    let slugTouched=false;slug.addEventListener('input',()=>slugTouched=true);
    function slugify(v){return v.toLowerCase().replace(/^https?:\/\//,'').replace(/^www\./,'').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');}
    ...
    function data(){
      const fd=new FormData(form);
      return {name:fd.get('name').trim(),domain:fd.get('domain').trim().replace(/^https?:\/\//,'').replace(/\/$/,''),slug:fd.get('slug').trim(),
        siteType:fd.get('siteType'),protocol:fd.get('protocol'),host:fd.get('host').trim(),port:Number(fd.get('port')),username:fd.get('username').trim(),
        password:fd.get('password'),remoteRoot:fd.get('remoteRoot').trim(),backupEnabled:form.backupEnabled.checked,
        backupIntervalSeconds:Math.round(Number(fd.get('backupHours'))*3600),backupMaxFiles:Number(fd.get('backupMaxFiles')),
        monitorEnabled:form.monitorEnabled.checked,monitorIntervalSeconds:Number(fd.get('monitorIntervalSeconds')),
        monitorFailureThreshold:Number(fd.get('monitorFailureThreshold')),sslWarnDays:Number(fd.get('sslWarnDays'))};
    }
    testConnection.onclick=async()=>{connectionStatus.textContent='Verbinde…';connectionFiles.innerHTML='';const r=await fetch('/api/site-connection-test',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(data())});const x=await r.json();if(!r.ok){connectionStatus.textContent='✗ '+(x.message||x.error||JSON.stringify(x));return;}connectionStatus.textContent='✓ Verbindung erfolgreich';connectionFiles.innerHTML=x.entries.length?'<strong>Gefundene Einträge:</strong> '+x.entries.map(e=>e.type==='directory'?'📁 '+e.name:'📄 '+e.name).join(' · '):'<strong>Verbindung erfolgreich.</strong> Der Ordner ist leer.';};
    form.onsubmit=async e=>{e.preventDefault();saveSiteStatus.textContent='Lege Website an…';const r=await fetch('/api/sites',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(data())});const x=await r.json();if(!r.ok){saveSiteStatus.textContent='Fehler: '+(x.message||x.error||JSON.stringify(x));return;}location.href='/sites/'+encodeURIComponent(x.slug);};
    </script>`);
}
...
async function start(){let databaseReady=false,databaseError=null;try{await migrate();await loadSavedConfig();databaseReady=true;}catch(e){databaseError=String(e?.message||e);console.error('database startup',e);}const app=Fastify({logger:true,bodyLimit:8*1024*1024});const missingConfig=()=>[['SITEOPS_MASTER_KEY',cfg.masterKey],['MCP_API_TOKEN',cfg.mcpToken],['DASHBOARD_USER',cfg.dashboardUser],['DASHBOARD_PASSWORD',cfg.dashboardPassword],['DB_USER',cfg.databaseUrl||cfg.dbUser],['DB_NAME',cfg.databaseUrl||cfg.dbName]].filter(([,v])=>!v).map(([k])=>k);app.get('/health',async(_req,reply)=>{const missing=missingConfig(),ok=databaseReady&&missing.length===0;return reply.code(ok?200:503).send({status:ok?'ok':'degraded',version:'0.5.0',port:cfg.port,database:{engine:'mysql',ready:databaseReady,error:databaseError},backup:{configured:Boolean(cfg.githubBackupRepo&&cfg.githubBackupToken),repository:cfg.githubBackupRepo||null},baseUrl:cfg.publicBaseUrl,missingConfig:missing,worker:databaseReady?'ok':'paused',time:new Date().toISOString()});});app.get('/assets/app.css',async(_r,reply)=>reply.type('text/css').send(await readFile(new URL('./public/app.css',import.meta.url),'utf8')));app.addHook('onRequest',async(req,reply)=>{if(req.url==='/health'||req.url.startsWith('/assets/'))return;if(req.url.startsWith('/mcp')){if(!mcpAuth(req,reply))return reply;}else if(!dashboardAuth(req,reply))return reply;});const handler=createMcpHandler(()=>mcpServer()),nodeHandler=toNodeHandler(handler);app.all('/mcp',async(req,reply)=>nodeHandler(req.raw,reply.raw,req.body));app.get('/',async(_r,reply)=>reply.type('text/html').send(await dashboard()));app.get('/api/sites',async()=>listSites());
const siteCreateSchema=z.object({slug:z.string().regex(/^[a-z0-9-]+$/),name:z.string().min(1),domain:z.string().min(1),protocol:z.enum(['sftp','ftps','ftp']),host:z.string().min(1),port:z.coerce.number().int().min(1).max(65535),username:z.string().min(1),password:z.string().optional(),privateKey:z.string().optional(),passphrase:z.string().optional(),remoteRoot:z.string().min(1),siteType:z.enum(['wordpress','php','static']).default('php'),monitorUrl:z.string().url().optional(),backupEnabled:z.boolean().optional(),backupIntervalSeconds:z.coerce.number().int().min(900).max(2592000).optional(),backupMaxFiles:z.coerce.number().int().min(100).max(200000).optional(),monitorEnabled:z.boolean().optional(),monitorIntervalSeconds:z.coerce.number().int().min(30).max(86400).optional(),monitorFailureThreshold:z.coerce.number().int().min(1).max(20).optional(),sslWarnDays:z.coerce.number().int().min(1).max(365).optional()});
...
app.post('/api/settings/github-test',async()=>{backupRepoChecked=false;await ensureBackupRepository();const r=await gh('');return{ok:true,repository:r.full_name,private:r.private,defaultBranch:r.default_branch,branch:cfg.backupBranch};});
