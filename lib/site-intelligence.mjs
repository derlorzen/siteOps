...
export async function siteIntelligenceAudit(baseUrl,{userAgent='Lorzen-SiteOps-Intelligence/0.9'}={}){
  ...
  const security=securityHeaderAudit(home.headers,normalized.protocol);
  ...
  const overallScore=Math.round((security.score+aiScore+domainScore)/3);
  return{
    ...
    scores:{overall:overallScore,security:security.score,aiSearch:aiScore,domain:domainScore},
    security:{...security,securityTxt},
