...
async function executeStep(page,step,baseUrl){
  ...
  if(action==='goto')await page.goto(sameHost(step.url||baseUrl,baseUrl),{waitUntil:step.waitUntil||'domcontentloaded'});
  ...
  else if(action==='assertText'){const text=await resolveSelector(page,step.selector).innerText();if(!text.includes(String(step.text??'')))throw new Error('Expected text not found in '+step.selector);}
  else if(action==='assertVisible'){if(!await resolveSelector(page,step.selector).isVisible())throw new Error('Expected element is not visible: '+step.selector);}
  else if(action==='assertValue'){const actual=await resolveSelector(page,step.selector).inputValue();if(actual!==String(step.value??''))throw new Error('Value assertion failed for '+step.selector+'. Expected '+String(step.value??'')+', got '+actual);}
  else if(action==='assertAttribute'){const actual=await resolveSelector(page,step.selector).getAttribute(String(step.name||''));if(actual!==String(step.value??''))throw new Error('Attribute assertion failed for '+step.selector+'['+step.name+']');}
  else if(action==='assertCount'){const actual=await page.locator(step.selector).count(),expected=Number(step.count);if(actual!==expected)throw new Error('Count assertion failed for '+step.selector+'. Expected '+expected+', got '+actual);}
  else if(action==='assertUrl'){const actual=page.url(),expected=String(step.contains||step.url||'');if(!actual.includes(expected))throw new Error('URL assertion failed. Expected to contain '+expected+', got '+actual);}
  else if(action==='assertTitle'){const actual=await page.title(),expected=String(step.contains||step.title||'');if(!actual.includes(expected))throw new Error('Title assertion failed. Expected to contain '+expected+', got '+actual);}
...
function comparePng(current,baseline){
  ...
  if(a.width!==b.width||a.height!==b.height)return{mismatch:1,width:a.width,height:a.height,dimensionMismatch:true};
  const diff=new PNG({width:a.width,height:a.height});
  const pixels=pixelmatch(a.data,b.data,diff.data,a.width,a.height,{threshold:.12,includeAA:false});
  return{mismatch:pixels/(a.width*a.height),pixels,width:a.width,height:a.height,dimensionMismatch:false};
}
app.get('/health',async()=>({status:'ok',service:'siteops-browser-runner',version:'1.0.0'}));
app.get('/auth-test',async(req,reply)=>{if(!auth(req,reply))return;return{ok:true,version:'1.0.0'};});
app.post('/run',async(req,reply)=>{
  if(!auth(req,reply))return;
  const body=req.body||{},baseUrl=String(body.baseUrl||'');
  ...
  const consoleErrors=[],pageErrors=[],failedRequests=[],httpErrors=[],stepResults=[],started=Date.now();
  ...
  const ok=!error;
  return{ok,error,durationMs:Date.now()-started,finalUrl:lastUrl,steps:stepResults,consoleErrors:consoleErrors.slice(0,50),pageErrors:pageErrors.slice(0,50),failedRequests:failedRequests.slice(0,100),httpErrors:httpErrors.slice(0,100),visual,screenshotHash:screenshot?crypto.createHash('sha256').update(screenshot).digest('hex'):null,screenshotBase64:screenshot&&((body.captureScreenshot)||!ok)?screenshot.toString('base64'):null};
});
